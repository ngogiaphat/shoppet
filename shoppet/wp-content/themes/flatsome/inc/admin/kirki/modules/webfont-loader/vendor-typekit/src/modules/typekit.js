...
goog.scope(function(){
  var Typekit = webfont.modules.Typekit, Font = webfont.Font;
  ...
  Typekit.prototype.load = function(onReady){
    ...
    var that = this;
    if(kitId){
      // Load the Typekit script. Once it is done loading we grab its configuration and use that to populate the fonts we should watch.
      this.domHelper_.loadScript(this.getScriptSrc(kitId), function(err){
        if(err){
          ...
          if(loadWindow['Typekit'] && loadWindow['Typekit']['config'] && loadWindow['Typekit']['config']['fn']){
            var fn = loadWindow['Typekit']['config']['fn'], fonts = [];
            for(var i = 0; i < fn.length; i += 2){
              var font = fn[i], variations = fn[i + 1];
              for(var j = 0; j < variations.length; j++){
                fonts.push(new Font(font, variations[j]));
              }
            }
            // Kick off font loading but disable font events so we don't duplicate font watching.
            try {
