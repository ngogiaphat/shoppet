...
(function (window, $){
	...
	/**
	 * Create missing variables and have default `classPrefix` overridden to avoid issues. `media` is now a fake wrapper needed to simplify manipulation of various media types,
	 * so in order to access the `video` or `audio` tag, use `media.originalNode` or `player.node`; player.container` used to be jQuery but now is a HTML element, and many elements inside
	 * the player rely on it being a HTML now, so its conversion is difficult; however, a `player.$container` new variable has been added to be used as jQuery object
	*/
	...
	// Add jQuery ONLY to most of custom features' arguments for backward compatibility; default features rely 100% on the arguments being HTML elements to work properly
